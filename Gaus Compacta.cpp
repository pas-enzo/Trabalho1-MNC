...
int somatorioB(int i, int j , int B[], int L[][max], int BU[]){		//somatorio de B
	int cont=0, k;
	for(k=0 ; k < i ; k++){
		cont+=L[i][k] * BU[k]; 
	}
...
void BU(int i, int j , int n, int B[] , int L[][max], int BU[]){		//Matriz de B
		if(i==0)BU[i]=B[i];
		
		else{
		BU[i]= B[i]- somatorioB(i, j, B, L ,BU); 
	}

}

void MGC(int t, int M[][10], int X[], int B[]){		//matriz de gaus compacta
	int i, j;	//operadores linha e coluna
	int U[max][max], L[max][max], Bu[max];	//matriz U e L
